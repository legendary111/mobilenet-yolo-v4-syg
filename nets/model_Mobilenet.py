...
def yolo_body(inputs, num_classes):
    ...
    Layer Nanem: global_average_pooling2d_1 Output: Tensor("global_average_pooling2d_1/Mean:0", shape=(?, 1024), 
    dtype=float32)
...
def yolo_eval(yolo_outputs,
    ...
    for e in range(num_layers):
        _boxes, _box_scores = yolo_boxes_and_scores(yolo_outputs[e],
                                                    anchors[anchor_mask[e]], num_classes, input_shape, image_shape)
        boxes.append(_boxes)
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    grid_shapes = [K.cast(K.shape(yolo_outputs[e])[1:3], K.dtype(y_true[0])) for e in range(num_layers)]
    loss = 0
    ...
    for e in range(num_layers):
        ...
        raw_true_xy = y_true[e][..., :2] * grid_shapes[e][::-1] - grid
        raw_true_wh = K.log(y_true[e][..., 2:4] / anchors[anchor_mask[e]] * input_shape[::-1])
